# MODIFIED from the Hermes Agent bundled `google-workspace` skill (c) Nous Research, MIT.
# Modifications (reference build): two-account per-op routing, --attach/--html send,
# calendar_list attendees. Install by OVERWRITING the bundled scripts after platform
# install; platform updates stash local changes — re-apply from this repo afterward.
...
# --- Two-account routing (operator@ reads vs agent@ send/Drive) ---
# No `gws` binary is installed, so the Python google-api-client path is the only
# active backend; _active_token_path() (used by get_credentials) is the single
# credential chokepoint. The account is selected per-invocation in main().
TOKEN_PATH_READ = HERMES_HOME / "google_token_read.json"   # operator@: gmail/calendar/tasks readonly
_TOKEN_PATHS = {"read": TOKEN_PATH_READ, "send": TOKEN_PATH}  # send = agent@ (google_token.json)
# Ops that use the agent@ token: send/reply, calendar (agent@ reads the operator's
# shared calendar and is the identity for future invites/suggestions), and all
# drive. Everything else (gmail/tasks reads) uses the operator@ token.
_SEND_OPS = {
    ("gmail", "send"), ("gmail", "reply"), ("gmail", "modify"),
    ("calendar", "list"), ("calendar", "create"), ("calendar", "delete"),
    ("drive", "search"), ("drive", "get"), ("drive", "upload"), ("drive", "download"),
    ("drive", "create-folder"), ("drive", "share"), ("drive", "delete"),
}
_ACTIVE_ACCOUNT = "read"


def _active_token_path():
    return _TOKEN_PATHS.get(_ACTIVE_ACCOUNT, TOKEN_PATH)
...
def _ensure_authenticated():
    if not _active_token_path().exists():
        print(f"Not authenticated for the '{_ACTIVE_ACCOUNT}' account. Run setup first:", file=sys.stderr)
        print(f"  python {Path(__file__).parent / 'setup.py'} --account {_ACTIVE_ACCOUNT} --auth-url", file=sys.stderr)
        sys.exit(1)
...
def get_credentials():
    """Load and refresh credentials from the active account's token file."""
    _ensure_authenticated()

    from google.oauth2.credentials import Credentials
    from google.auth.transport.requests import Request

    tp = _active_token_path()
    creds = Credentials.from_authorized_user_file(str(tp), _stored_token_scopes())
            ...
            events.append({
                ...
                "attendees": [
                    {"email": a.get("email", ""), "responseStatus": a.get("responseStatus", ""),
                     "displayName": a.get("displayName", ""), "self": a.get("self", False),
...
# =========================================================================
# Tasks (read-only; operator@ account)
# =========================================================================
    ...
    for tl in lists:
        items = (
            ...
            .list(
                tasklist=tl["id"],
                maxResults=args.max,
                showCompleted=getattr(args, "show_completed", False),
                showHidden=False,
            )
            .execute()
            .get("items", [])
...
def main():
    ...
    # Route credentials: write/act ops use the send (agent@) token; all else
