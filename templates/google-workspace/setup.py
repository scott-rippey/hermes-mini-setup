...
# Two-account model: operator@ reads the operator's actual mail/calendar/tasks; agent@
# sends email and writes backups to Drive. Each account is consented separately
# (setup.py --account read|send) into its own token file. Tasks has no sharing,
# so reading the operator's to-dos *requires* the operator@ token.
SCOPES_BY_ACCOUNT = {
...
TOKEN_PATH_BY_ACCOUNT = {
    "read": HERMES_HOME / "google_token_read.json",
    "send": HERMES_HOME / "google_token.json",
}
PENDING_BY_ACCOUNT = {
    "read": HERMES_HOME / "google_oauth_pending_read.json",
    "send": HERMES_HOME / "google_oauth_pending_send.json",
}
# Defaults (overridden per-invocation in main() from --account).
SCOPES = SCOPES_BY_ACCOUNT["send"]
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_hermes_home()}/{TOKEN_PATH.name}")
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["read", "send"],
        default="send",
        help="Account to operate on: 'read' = operator@ (gmail/calendar/tasks readonly), "
             "'send' = agent@ (gmail.send + drive.file). Default: send.",
    )
    ...
    # Point the shared globals at the selected account; every command below
    # (check / auth-url / auth-code / revoke) reads these at call time.
    global SCOPES, TOKEN_PATH, PENDING_AUTH_PATH
    SCOPES = SCOPES_BY_ACCOUNT[args.account]
    TOKEN_PATH = TOKEN_PATH_BY_ACCOUNT[args.account]
    PENDING_AUTH_PATH = PENDING_BY_ACCOUNT[args.account]
