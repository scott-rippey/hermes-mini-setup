...
5. **Schedule** nightly via `templates/launchd/job.plist.template` (reference: `ai.hermes.meeting-reports` @ 10pm, headless `hermes -z`), after the 9pm docs-sync. Gate: process one real meeting end-to-end — KB row exists, report appears in the next brief.
