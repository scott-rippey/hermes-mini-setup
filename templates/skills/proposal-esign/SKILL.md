...
Omit `--live` for a test-mode rehearsal (creates the document, sends nothing, bills nothing — the recipient address still receives a banner-wrapped PREVIEW email, useful for checking branding). The send is recorded in `~/.hermes/signwell/pending.json` for tracking. **The send also auto-files the drafting Markdown into the KB** under `--customer` as `status: sent` (via `~/.hermes/scripts/kb_file.py`; the result is in the send output under `kb` — if it says the Markdown was not found beside the PDF, save it there and run `kb_file.py` yourself).

**File at render time, too:** as soon as a proposal or contract is rendered (deliverable-export), run `python ~/.hermes/scripts/kb_file.py --md <draft.md> --customer <slug> --document-type proposal|contract --status draft` — no offer. Send and signature then re-file it as `sent` / `signed`.
...
A **15-min launchd poller** (`ai.hermes.signwell-poll`, runs `poll --notify`) watches pending docs in the background: on a signed doc it downloads the PDF to `~/{{AGENT_SLUG}}-outputs/<date>-<doc>-SIGNED.pdf` and files it into the KB as signed, and posts the result (with an @-mention) to the proposals channel — the target comes from `SIGNWELL_NOTIFY_CHANNEL` in `.env` (set it to the channel ID: rename-proof). **No email is sent — SignWell itself emails the operator the signed copy.** When a signature lands (via the cron's Slack post, or `"result": "signed"` from your own poll):
1. **Filing is automatic** (the operator's standing decision — everything this flow produces is filed under its customer without an offer): the poller re-files the drafting Markdown as `status: signed` with `signed: true`, `signed_date` (SignWell's `updated_at` of the Completed doc), `signer_name`/`signer_email`, `signwell_document_id`, `signed_pdf_path` (`type: reference`, `document_type: proposal|contract`; stage titles `[draft]` → `[sent]` → `[signed]`, each stage superseding the previous so history is kept and one stage is active). Its Slack post says whether the filing succeeded; on a failure, file by hand with `python ~/.hermes/scripts/kb_file.py --md <draft.md> --customer <slug> --document-type <kind> --status signed --signwell-id <id> --signed-pdf <path> --signed-date <iso>`. The filed content is always the drafting Markdown (never a PDF-back-conversion; signature tags are stripped by the helper).
