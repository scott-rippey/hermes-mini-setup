...
- **Solo appointments still get travel:** an event with a physical address but no other attendees skips the full prep (no one to research) and instead sends a compact deterministic reminder — title, time, place, **leave by** — same travel engine, no synthesis. No address and no attendees → silence, as before.
