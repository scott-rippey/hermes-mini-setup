...
- **Chat (all personas):** GPT-5.6 Sol via the **Codex OAuth** on a flat-rate ChatGPT plan — token counts are the signal, dollars are not. Headless runs (`hermes -z`) ride the same path: brief synthesis, call reports, change narratives.
- **Fallback chain (recommended; the installer prompts for it in Phase 3):** `fallback_providers: [{provider: anthropic, model: claude-sonnet-5}]` with the console key in `.env` as **BOTH `ANTHROPIC_API_KEY` AND `ANTHROPIC_TOKEN` (same value)** — the platform's Anthropic credential ladder checks `ANTHROPIC_TOKEN` first and `ANTHROPIC_API_KEY` LAST, after Claude Code's own login; on any box where Claude Code is signed in (i.e. every box mid-install), the API key alone loses to CC's consumer OAuth and every fallback call fails HTTP 400 ("third-party apps draw from extra usage"). `sk-ant-api*` keys are classified never-OAuth, so the duplicate at priority 1 passes through untouched. — on primary failure (429 after retries, 5xx outage, 401/403, 404, malformed responses) the turn finishes on the fallback with context preserved; **turn-scoped**, so every new message retries the primary first and outages self-heal. Detection: the ops digest's by-model token table (fallback-provider tokens appearing = it fired). Force-test after wiring: `hermes -m <nonexistent-model> -z "which model family are you?"` → the answer should come from the fallback.
- **Aux:** a small model for web extraction (reference: `gpt-5.6-luna`). **Embeddings:** OpenAI `text-embedding-3-large` on a separate, metered API key that exists *only* for embeddings (pennies/month).
