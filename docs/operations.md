...
Pre: `updates.pre_update_backup: true` (set it in Phase 2 and never unset) · commit the ledger · **patch lifeboat** — if you carry local commits in the platform clone, capture them first: `git -C ~/.hermes/hermes-agent format-patch <upstream-base>..<carried-commit> -o ~/backups/platform-patches-<date>/` (see the pin mechanic below — the updater can destroy carried commits) · snapshot config.yaml for the post-migration semantic diff · `hermes skills list-modified`.

**The version pin is manual — `hermes update` cannot target a tag.** It tracks `origin/main`, and on diverged history (any carried local commit) it runs `git reset --hard origin/main`, destroying the carried commit — intentional updater behavior, not a bug. The pinned-jump recipe instead: `git fetch --tags origin` → resolve the release tag (date-form, e.g. `v2026.8.19`) → `git checkout main && git reset --hard <tag>` (branch stays `main`, pointed at the tag — an ancestor of origin/main, so the next jump works the same way) → `./venv/bin/python -m pip install -e ".[all]"` → `hermes doctor --fix`. Re-applied patch commits re-diverge main; that is the steady state.

**Version-jump review BEFORE running the update** — read every release note between the pinned and target versions for **default-behavior flips**. Known ones on the v0.17→v0.19 path (the reference build ran this jump live): **v0.19 made LLM-judged "smart approvals" the platform default** — an explicit `approvals.mode: manual` in config.yaml overrides it, but re-verify (plus `approvals.cron_mode: deny`) after every update before the gateway goes back up. v0.19 also ships **new skills enabled** (`docx`/`pdf`/`xlsx` — the reference build adopted all three deliberately) and **new toolsets inside bundles** (`project`, `video`, `video_gen`); v0.18 adds `/goal`, `/learn`, `/journey` — check `/learn`-created skills against default-deny. The durable gateway delivery ledger (at-least-once Slack delivery) is a reason TO take the update. **Known ones on the v0.19→v0.20 path (the reference build ran this jump live):** Node ≥26 required (the CLI TUI is Node-based); **five new toolsets ship enabled** (`a2a`, `desktop_ui`, `browser-use`, `browser-cdp`, `bfl` — see [security.md](security.md)); **13 new bundled skills seed themselves ENABLED at the first gateway start on the new version** (see the reseed trap in the list below); Bot Mode is a desktop-app plugin (plugins are opt-in — verify none enabled with `hermes plugins list`); the keyless web-search ring arrives default-on (`web.keyless_fallback: false` to keep default-deny); the compaction-notice patch retires — `compression.codex_gpt55_autoraise_notice: false` now does it in config; and the platform moves to the MCP 2.x SDK (a `mcp>=1.28,<2` KB server keeps working — the wire protocol interoperates — and the server's own v2 rewrite becomes possible). **Two mechanics from the live run:** the update **auto-restarts the gateway when it finishes** — patches and config checks land after that restart, so finish the whole list and restart again; and `hermes doctor` may flag an outdated `_config_version` — run `hermes doctor --fix`, then audit its rewrite **semantically** (parse pre/post YAML and diff values; the rewriter also re-wraps long strings, which is noise — renames and default flips are the signal).

Update, then re-apply:
1. **google-workspace modified scripts** ← [templates/google-workspace/](../templates/google-workspace/)
2. **Skills reseed check — run it AFTER the first gateway start on the new version:** new bundled skills are seeded enabled at gateway startup, so a list taken before that restart gives a **false pass** (the reference build's v0.20 jump seeded 13 new skills enabled, caught only by a post-restart recount). Count the enabled rows programmatically, add arrivals to `skills.disabled`, restart, recount. `agent.disabled_toolsets` survives (config), but re-scan the toolset registry for never-discussed new toolsets
3. **Optional core hardening**, if you adopted it (both are small, platform-core edits — re-do after updates):
   ...
   - *Notice gate:* RETIRED as a patch on v0.20+ — set `compression.codex_gpt55_autoraise_notice: false` (config, update-proof; keeps the context-threshold autoraise, hides the banner).
