...
| Dashboard branding | SignWell Settings → Branding | Logo: use a **300×60 self-backgrounded lockup bar** (icon + wordmark on a dark rounded bar, transparent corners) — theme-proof on light webmail and dark-mode mobile, where a transparent logo fails one or the other. **Web-plan-gated:** the logo banner + email signature on signature-request emails require the **Business** web plan (the trial shows them, then they lapse); the skill's `custom_requester_name`/`custom_requester_email` API fields restore the from-**name** and reply-to on any plan and stamp the requester into the audit file. The actual sending address is always `signwelldocs@signwell.com` — no tier changes that. The PDF's own branding is yours regardless |
...
5. **Signature lands:** the poller detects it, saves the PDF, files the drafting Markdown into the KB as `[signed]` (superseding the `[sent]` stage), and posts the result to #proposals (`hermes send` — deterministic, no AI), prefixed with `<@member-id>` read from `SLACK_ALLOWED_USERS` in `.env` — a hard notification, self-configuring on any box. Interactive polls omit `--notify` so agent and cron never double-post.
6. **Filing is automatic across the whole flow** (the operator's standing decision): `kb_file.py` files the drafting Markdown under the send-time customer at render (`[draft]`), at send (`[sent]`, by `signwell.py send`) and at signature (`[signed]`, by the poller) — `type: reference`, `document_type: proposal|contract`, SignWell id, signer, dates, signed-PDF path — each stage superseding the previous, so history is kept and exactly one stage is active. Every supersede is announced in the ops Slack channel. A failed filing is reported in the poller's Slack post and fixed by hand with the same helper.
