...
**Governing invariant: no document's status ever changes without the operator's explicit decision, and no retirement is ever silent.** The system flags; the operator decides. No timers, no auto-decay, no auto-merge; hard delete is a manual psql action only. Two sanctioned automatic paths exist because the operator decided them once: the proposals/contracts flow (each stage supersedes the previous — see proposal-esign.md) and the nightly GitHub docs sync. Every supersede, from any path, posts one line to the ops Slack channel (the RAG server's `_announce` → `scripts/system_notify.sh`) and stamps `metadata.superseded_at`, so a retirement can always be seen and undone (restore = status back to active).
...
- **Filing helper:** [`templates/scripts/kb_file.py.template`](../templates/scripts/kb_file.py.template) — files a rendered deliverable's drafting Markdown into the KB through the server's own `_store` (runs under the mcp-rag venv; stage titles `[draft]/[sent]/[signed]` superseding each other). Used by the proposals persona, `signwell.py send`, and the signature poller.
...
| Proposals & contracts flow (`kb_file.py`) | **Sanctioned deterministic auto-write** — the drafting Markdown files itself at render (`[draft]`), send (`[sent]`) and signature (`[signed]`), each stage superseding the previous; announced in the ops channel |
