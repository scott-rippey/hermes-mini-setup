...
| `google_token.json` | **agent@your-domain** | `gmail.send`, `gmail.readonly`, `calendar.readonly`, `calendar.events`, `drive.file` | Sending email *as the agent*, **reading the agent's own inbox** (`--account send gmail search …` on demand; hourly `agent_inbox.py` watch posts new mail to the home channel, flags replies to something the agent sent, never replies itself), reading the operator's **shared** calendar, writing Drive (backups) |
...
4. **Auth both lanes:** `setup.py --account read` (as the operator) and `--account send` (as the agent). Adding a scope later = re-auth that account. The agent lane carries `gmail.readonly` so the agent can see its OWN mailbox — nobody opens that account by hand, so without it vendor mail and replies to the agent's outbound sit unseen (`templates/scripts/agent_inbox.py.template`, hourly).
