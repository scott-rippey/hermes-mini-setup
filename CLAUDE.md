...
`sql/schema.sql` (apply verbatim) · `sql/seeds.sql` (generated in Phase 0) · `templates/` (launchd plists, scripts, skills, identity — instantiate by replacing `{{PLACEHOLDERS}}` from `setup/answers.md`) · `docs/` (per-subsystem deep dives + the operator's runbook) · `setup/` (your working state: answers, progress — gitignored)
